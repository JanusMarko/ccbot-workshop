"""Shared utility functions used across CCWeb modules.

Provides:
  - ccweb_dir(): resolve config directory from CCWEB_DIR env var.
...
CCWEB_DIR_ENV = "CCWEB_DIR"


def ccweb_dir() -> Path:
    """Resolve config directory from CCWEB_DIR env var or default ~/.ccweb."""
    raw = os.environ.get(CCWEB_DIR_ENV, "")
    return Path(raw) if raw else Path.home() / ".ccweb"
